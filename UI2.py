...
class mainWindow(QWidget):
    ...
    message_change = QtCore.pyqtSignal(str)

    class Chat(QLabel):
        def __int__(self, chat_data):
            QLabel.__init__(self, "test")

    @QtCore.pyqtSlot(str)
    def updateChat(self, data):
        self.chat_print.verticalScrollBar().setSliderPosition(self.chat_print.verticalScrollBar().maximum())
        d = self.Chat("test")
        self.vbox.addWidget(d)
    ...
    def mainDesign(self):
        ...
        self.chat_print = QScrollArea()


        self.chat_print.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOn)
        self.chat_print.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        self.chat_print.setWidgetResizable(True)
        self.chat_print.setWidget(self.widget)
        self.message_change.connect(self.updateChat)
    ...
    def handle_messages(self):
        ...
        while not flag:
            ...
            if msg["action"] == "send_msg":
                self.message_change.emit(json.dumps(msg))
